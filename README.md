さらに文章の最下部に詳しく端的な説明があります。
形型ナノスパイラル量子デバイスアンテナ単独開発者の星澤宜嗣(ほしざわのりつぐ)と申し上げます。どうかよろしくお願い申し上げます🙇
Fainal Gyanbler 13X(FGDX)(仮称） ＝　kiguugames
　下記に技術参考図書表示してあります。
kiguugamesはコマンドプロンプト(Windows)やターミナル(Mac)等のコマンド入力(ゲームは音楽ファイル、画像フォルダととが入ってる同一フォルダ内に"cd そのフォルダ名のあるエクスプローラ上のアドレス"にてエクスプローラーのフォルダアドレスをカレントディレクトリにした後に、(僕はコマンドプロンプト上のデフォルトディレクトリ(ファイル実行時のフォルダあるのコンピュータ上のアドレス)のUsers(表記される氏名に当たるセットアップ時に決めるコンピュータ名は半角英数字にてのみ対応、平仮名漢字は使用するとエラーが起きるからPCリカバリーにて半角英数字のみに設定し直しました、私は昔の若い頃に。非常に大事です。)ディレクトリ内にkiguuフォルダ等をコピーアンドペースト置いております。操作が楽楽だからです。)python3 ファイル名.py or python ファイル名.pyコマンド入力でドス窓みたいな環境下でコンピュータ開発に慣れるために安心して安全に遊ぶゲームです。コード記述のエディターはサクラエディタ"　https://sakura-editor.github.io/ "
様を私は愛用してます。pythonの環境インストールが必要です。スマートフォンでは、a-shellアプリを僕はコンパイラーとして利用させていただいております。（kiguunh フォルダすべて　~otonashi. pyのファイルのみとすべてのsetup系. pyのファイルの起動を確認済みです。下記のplaysoundやpygameのインストールはスマートフォンでは必要ありません。できないので）
...
～以上導入方法以下mac導入方法～
Macコンピューター上では（注意メモ書き） 下記の参照URLにて詳しく載ってました。　Macでは、ターミナルにてHomebrewをインストール後にbrew install python@3.10 をコマンド(brew listにてpythonバージョン確認できます。brew uninstall python@3.?にてアンインストール可能です。)実行後にpython3.10インストールしてください。その後にpython3 get-pip.pyを実行してpythonモジュールインストールに必要なpipをインストール後、コマンドに順に、pip install playsound==1.2.2 にて、pip install pygame にてpygameとplaysoundモジュールがそれぞれですね私も使えるようになりまして。〜otonashi.pyでpython3 ファイル名.py で動きますが、モジュールのplaysoundやpygameは動かすこと可能だと実際に確認済みです。実行はcd フォルダ名　実行の後　コマンド欄にpython3 ファイル名.py で実行できます。確認済みです。インターネットでいろいろと調べてHomebrewを入れまして（下記に説明サイトを参考にしてあります。説明サイト様や説明してくれたAI様社会に大変にお世話になっております。感謝謝意。どうも有難うございますごめんなさい。）python@3.10を入れてpipを入れ、モジュールをインストールしましたら、あっさりと上記の如くゲームプレイができました。社会や開発環境に感謝謝意どうもありがとうございますごめんなさい。　Windows8.1(モジュールが動きます)にて開発させて頂きました感謝謝意どうもありがとうございます。
...
2026.02.23(月)kiguunhフォルダにてraceotonashi.pyのレース勝ち負け選択のあたり時の獲得ポイント各々10倍が反映されておらずでした。修正いたしました。どうかよろしくお願い申し上げます。
2026.02.23(月)kiguunhフォルダにkiguu.py 追加いたしました。どうかよろしくお願い申し上げます。
2026.02.22(日)22:16 raceにおける一位選択に当たった時の倍率を１０倍づつ増やしてあげました。どうかよろしくお願い申し上げます。
 2025.09.16(火）19:58 エラーが発見されましたが、原因はkekka=0の宣言が抜けておりましたすべてrace がファイル名につくフォルダを直しました。安心して遊べます。ごめんなさい。どうかよろしくお願い申し上げます。18:36　setuprace. py をはじめといたしましたraceから始めるすべてのファイルのゲーム性に変更を施しました。内容はポイントを運用できるゲームをすべてにおいて付加いたしました始める際に一度だけsetuprace. pyの実行をどうかよろしくお願い申し上げます。どうかよろしくおねがいもうしあげます。いつもどうもありがとうございますごめんなさい。どうかYoutubeとXの応援のほど、どうかよろしくお願い申し上げます。
2025.07.12(土)15:56 jankensugorokuikiru. py jankensugorokuikiruotonashi. pyのポイント加算時の表示がおかしいのに動画を見まして今気づきなおしました。
2025.07.12(土)7:19 jankensugorokuikiru. py jankensugorokuikiruotonashi. pyのゲームのrobo63様のポイント繁栄がなされていなかった(houuupエラーを発見いたしました)のでコード追加修正いたしました。どうかよろしくおねがいもうしあげます。いつもどうもありがとうございますごめんなさい。感謝謝意。
2025.07.11(金)23:00 jankensugorokuikiru. py jankensugorokuikiruotonashi. pyのゲームのrobo63様のポイント繁栄がなされていなかったのでコード追加修正いたしました。どうかよろしくおねがいもうしあげます。いつもどうもありがとうございますごめんなさい。感謝謝意。
2025.07.11(金)23:00 jankensugorokuikiru. py jankensugorokuikiruotonashi. pyのゲームを面白くいたしました。どうかよろしくおねがいもうしあげます。いつもどうもありがとうございますごめんなさい。感謝謝意。
2025.07.08(火).12:53
...
ISBN 978-4-8007-1239-4 
C3055
¥2680E
 
Python言語の基本から仕事のやり方まで　独学プログラマー　コーリー・アリソフ　清水川貴之　監訳　日経BP社
...
[https://prog-8.com/docs/python-env)](https://prog-8.com/docs/python-env)imac mini python環境参照しました。pip3やpython３インストールにて必須な説明が書いてありました。imac環境では、僕は上記のURLの説明の如くpyenv install 3.10.17 (pyenv uninstall 3.10.17
  ...
  コマンドラインにて　pip3 list や　brew list でインストールしたpip3やbrew内のインストールしたモジュールの一覧から確認が出来ます。
