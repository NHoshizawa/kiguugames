...
　下記に技術参考図書表示してあります。
kiguugamesはコマンドプロンプト(Windows)やターミナル(Mac)等のコマンド入力(ゲームは音楽ファイル、画像フォルダととが入ってる同一フォルダ内に"cd そのフォルダ名のあるエクスプローラ上のアドレス"にてエクスプローラーのフォルダアドレスをカレントディレクトリにした後に、(僕はコマンドプロンプト上のデフォルトディレクトリ(ファイル実行時のフォルダあるのコンピュータ上のアドレス)のUsers(表記される氏名に当たるコンピュータ名は半角英数字にてのみ対応、平仮名漢字は使用するとエラーが起きるからPCリカバリーにて英数字に設定し直しました、私は昔の若い頃に。非常に大事です。)ディレクトリ内にkiguuフォルダ等をコピーアンドペースト置いております。操作が楽楽だからです。)python3 ファイル名.py or python ファイル名.pyコマンド入力でドス窓みたいな環境下でコンピュータ開発に慣れるために安心して安全に遊ぶゲームです。コード記述のエディターはサクラエディタ"　https://sakura-editor.github.io/ "
