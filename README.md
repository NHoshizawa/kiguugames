...
　下記に技術参考図書表示してあります。
kiguugamesはコマンドプロンプト(Windows)やターミナル(Mac)等のコマンド入力(ゲームは音楽ファイル、画像フォルダととが入ってる同一フォルダ内のcdそのフォルダ名にてエクスプローラーのフォルダアドレスをカレントディレクトリにした後に僕はusersディレクトリ内に
kiguuフォルダを置いております。楽だからです。)(python3 or python ファイル名.py)でドス窓みたいな環境下でコンピュータ開発に慣れるために安心して安全に遊ぶゲームです。コード記述のエディターはサクラエディタ"　https://sakura-editor.github.io/ "
