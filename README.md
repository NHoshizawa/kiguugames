...
～以上導入方法以下mac導入方法～
Macコンピューター上では（注意メモ書き） Macでは、ターミナルにてHomebrewをインストール後にbrew install python@3.10 をコマンド実行後にpython３ get-pip.pyを実行して、pip install playsound==1.2.2 にてpip install pygame にてpygameとplaysoundモジュールが私も使えるようになりまして。〜otonashi.pyでpython3 ファイル名.py で動きますが、モジュールのplaysoundやpygameは動かすこと可能だと実際に確認済みです。インターネットでいろいろと調べてHomebrewを入れましてpython@3.10を入れてpipを入れ、モジュールをインストールしましたら、あっさりと上記の如くゲームプレイができました。感謝謝意どうもありがとうございますごめんなさい。　Windows8.1(モジュールが動きます)にて開発させて頂きました感謝謝意どうもありがとうございます。
