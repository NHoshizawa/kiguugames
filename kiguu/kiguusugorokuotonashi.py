...
def bonus(file1,file2,nokori):
	...
	points = hoyuup1 + hoyuup
	points2 = hoyuup2 - hoyuup
...
def game(player3):
	while True:
		...
		global Robo63_ichi
		if janken1=='p':
			#print(player3+"さんEnterを押すとサイコロを振れますよ")
			#playsound("sai1.mp3")
			r2 = awase()
			detame = awase2(r2)
			...
			if own_ichi == 30:
				...
				bonus(player3,filerobo63,nokori2)
			...
			#print(player3+"さんはじゃんけんに負けたのでサイコロを振れません")
			detame = 0
			own_ichi = own_ichi + int(detame)
			nokori = str(30 - own_ichi)
			nokori = int(nokori)
			...
			#print(player3+"さんEnterを押すとサイコロを振れます。じゃんけんであいこだったから出た目の2/3だよ")
			#playsound("sai1.mp3")
			r2 = awase()
			detame = awase2(r2)
		...
		if janken1=='p':
			...
			nokori2 = int(nokori2)
			if nokori2 > 0:
				print("Robo63ちゃんはあと残り～"+ str(nokori2) +"～こまです")
			else:
				print("Goal!!しました")
			if Robo63_ichi > 30:
				Robo63_ichi = 30
			...
			if Robo63_ichi ==30:
				...
				hunobonus(player3,player3,nokori)
