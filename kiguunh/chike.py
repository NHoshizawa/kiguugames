...
def ru(file6):
	if os.path.exists("./"+str(file6)+"kanri.pickle"): 
		...
		if answer in y:
			...
			print("How play's much in front of the other games on the true's game's decided factor!?→lose:1 or Win:2 even: 3 !?:")
			...
			if number == 2:
				...
				point8 = int(point8) * r
				#playsound("2bai.wav")
				print("very good chike(値気)!!" +str(r)+"倍の"+str(point8)+"pointになりました")
		...
		if answer in y:
			...
			if ans == 'true':
				...
				print("How play's much in front of the other games on the true's game's decided factor!?→lose:1 or Win:2 or even :3 !?:")
				number = input("select,lose→1 or win→2 even→3 1 or 2 or 3 !?")
				number = int(number)
				bet = int(bet)
				if bet <= int(point1) and bet >= 0:
					...
					if number == 2:
						...
						#playsound("2bai.wav")
						print("very good chike(値気)!!" +str(r)+"倍の"+str(point8)+"pointになりました。")
						print("if lose,bet's points is out of order.外れたら運用分は0ポイントになります。")
						with open(str(file6) +'.pickle', mode='wb') as f:
