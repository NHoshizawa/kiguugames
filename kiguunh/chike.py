...
def ru(file6):
	if os.path.exists("./"+str(file6)+"kanri.pickle"): 
		...
		if answer in y:
			...
			print("How play's much in front of the other games on the true's game's decided factor!?→lose:0 or Win:1 even: 3 !?:")
		...
		if answer in y:
			...
			if ans == 'true':
				...
				print("How play's much in front of the other games on the true's game's decided factor!?→lose:0 or Win:1 or even :3 !?:")
				number = input("select,lose→1 or win→2 even→3 1 or 2 or 3 !?")
				number = int(number)
				bet = int(bet)
